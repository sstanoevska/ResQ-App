...
#     update_emergency_contact, edit_doctor_profile, edit_patient_info, delete_doctor_patient
# )
    ...
    try:
        ...
        if alert_type == "doctor":
            for c in contacts:
                if c["contact_type"] == "doctor":
                    msg = (
                        f"ПОМОЩ!🚨 Вашият пациент {patient_name} се нуждае от вашата помощ! Моля, обадете се веднага на {patient_phone}!"
            ...
            for c in contacts:
                if c["contact_type"] != "doctor":  # Only send to non-doctors
                    text = (
                        f"{c['name']}, {patient_name} се нуждае от вашата помощ! 🚨 Моля, обадете се веднага на {patient_phone}!"
                        f"\n\n {c['name']}, {patient_name} needs your help!🚨 Please contact them immediately at : {patient_phone}!")
